                    ...
                    <div class="mb-12 lg:mb-0 lg:w-2/3 lg:pr-12">
                        ...
                        <form id="registerForm">
                            ...
                            <div class="flex space-x-7">
                                ...
                                <div class="mb-6 w-1/2">
                                    <label for="email"
                                        class="mb-1 block font-display text-sm text-jacarta-700 dark:text-white">Email<span
                                            class="text-red">*</span></label>
                                    <input name="email"
                            ...
                            <div class="flex space-x-7">
                                <div class="mb-6 w-1/2">
                                    <label
                                        class="mb-1 block font-display text-sm text-jacarta-800 dark:text-white">Mobile<span
                                            class="text-red">*</span></label>
                                    <input name="mobile"
                                        class="contact-form-input w-full rounded-lg border-jacarta-800 py-3 hover:ring-2 hover:ring-accent/10 focus:ring-accent dark:border-jacarta-800 dark:bg-jacarta-700 dark:text-white dark:placeholder:text-jacarta-300"
                                        id="mobile" type="text" required />
                                </div>
                            ...
                            <div class="flex space-x-7">
                                <div class="mb-6 w-1/2">
                                    <label class="mb-1 block font-display text-sm text-jacarta-700 dark:text-white">Confirm
                                        Password<span class="text-red">*</span></label>
                                    <input name="c_password"
                                        class="contact-form-input w-full rounded-lg border-jacarta-100 py-3 hover:ring-2 hover:ring-accent/10 focus:ring-accent dark:border-jacarta-800 dark:bg-jacarta-700 dark:text-white dark:placeholder:text-jacarta-300"
                                        id="c_password" type="password" required />
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            $('#registerForm').submit(function(e) {
                ...
                $.ajax({
                    url: '{{ url('/register') }}', // Replace with the correct login route
                    type: 'POST',
                    data: formData,
                    ...
                    success: function(response) {
                        ...
                        if (response.status === 1) {
                            console.log('success')
                            // Login successful, redirect to a new page or do something else
                            window.location.href =
                                '{{ url('/login') }}' // Replace with the desired URL after successful login
                        } else {
                            // Login failed, show an error message or do something else
                            $('#registerMsg').html(response.message);
                        }
                    },
                    error: function(xhr, status, error) {
                        // Handle AJAX errors if any
                        $('#registerMsg').html(xhr.responseJSON.message);
                    }
                });
            });
        });
